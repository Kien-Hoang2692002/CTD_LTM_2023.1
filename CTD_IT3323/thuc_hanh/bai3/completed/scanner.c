...
Token* readNumberOrFloat(void) {
  ...
  // Kiểm tra nếu ký tự tiếp theo là CHAR_PERIOD
  ...
  if (count == 1) {
    // Nếu chỉ có dấu chấm mà không có phần thập phân, báo lỗi
    error(ERR_INVALIDTYPE, lineNo, colNo);
    return makeToken(TK_NONE, lineNo, colNo);
  }

  token->string[count] = '\0';
  token->value = atof(token->string);  // Sử dụng atof để chuyển đổi sang số thực
  return token;
}
...
Token* getToken(void) {
  ...
  case CHAR_DIGIT: return readNumberOrFloat();
  case CHAR_PLUS: 
